...
type CourseThunk = AsyncThunk<any, { instructorId: string, options: LoadCourseOptions }, {}>
const useLoadInstructorCourses = ({ queryOptions, setLoading, setError }: Options) => {
    ...
    async (
      ...
      try {
        ...
        const res = await dispatch(
          loaderThunk({ instructorId: user.id, options: queryOptions })
        )
  ...
  const loadInstructorAllCourses = useCallback(
    () => fetchCourses(loadInstructorCourses, "Failed to load highest rated courses"),
    [fetchCourses]
  )
